...
function shimModel(target) {
  ...
  shimmer.massWrap(target, methods(Sequelize.version).findByPk, original => {
    return function batchedFindById(id, options = {}) {
      ...
      if (!options[EXPECTED_OPTIONS_KEY] || options.transaction || options.include || activeClsTransaction()) {
...
function shimBelongsTo(target) {
  ...
  shimmer.wrap(target, 'get', original => {
    return function batchedGetBelongsTo(instance, options = {}) {
      if (!options[EXPECTED_OPTIONS_KEY] || Array.isArray(instance) || options.where || options.include || options.transaction || activeClsTransaction()) {
        return original.apply(this, arguments);
      }
...
function shimHasMany(target) {
  ...
  shimmer.wrap(target, 'get', original => {
    return function bathedGetHasMany(instances, options = {}) {
      let isCount = false;
      if (!options[EXPECTED_OPTIONS_KEY] || options.include || options.transaction || options.separate || activeClsTransaction()) {
        return original.apply(this, arguments);
      }
...
function shimBelongsToMany(target) {
  ...
  shimmer.wrap(target, 'get', original => {
    return function bathedGetHasMany(instances, options = {}) {
      ...
      if (!options[EXPECTED_OPTIONS_KEY] || options.include || options.transaction || activeClsTransaction()) {
        return original.apply(this, arguments);
      }
